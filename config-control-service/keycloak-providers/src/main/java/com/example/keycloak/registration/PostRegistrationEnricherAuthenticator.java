...
/**
 * Post-registration authenticator that enriches user attributes after user
 * creation.
...
public class PostRegistrationEnricherAuthenticator implements Authenticator {
    ...
    private void enrichUserProfile(AuthenticationFlowContext context, UserModel user,
            UserProfileEnricher.EnrichedProfile profile) {
    ...
    private void resolveManagerRelationship(KeycloakSession session, RealmModel realm, UserModel user,
        ...
        try {
            ...
            if (manager != null) {
                ...
                logger.info("Set manager_id={} for user={} (manager={})", managerId, user.getUsername(),
                        managerUsername);
            } else {
