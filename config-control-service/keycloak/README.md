...
Other attributes (phone, department, etc.) are available via Admin API but not in tokens to minimize token size.

## User Profile Configuration

The User Profile is configured programmatically via Admin API in `keycloak-init.sh` with:

- **Validation Rules**: Pattern validation for manager_id (UUID), phone (E.164)
- **Permissions**: Admin-only edit for manager_id, department; User-editable for phone
...
- **admin2**: System Administrator 2
- **user1**: Team1 Lead - Manager for user2
- **user2**: Team1 Member - Reports to user1
- **user3**: Team2 Lead - Manager for user4
- **user4**: Team2 Member - Reports to user3
- **user5**: No team membership
