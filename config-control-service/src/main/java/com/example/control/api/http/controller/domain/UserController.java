...
import com.example.control.api.http.dto.domain.UserDtos;
import com.example.control.api.http.mapper.domain.UserApiMapper;
import com.example.control.application.service.helpers.UserPermissionsService;
...
public class UserController {
         ...
         */
        @GetMapping("/whoami")
        @Operation(summary = "Get current user information", description = """
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "User information retrieved successfully", content = @Content(schema = @Schema(implementation = UserDtos.MeResponse.class))),
        ...
        @Operation(summary = "Get user permissions and routes", description = """
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "User permissions retrieved successfully", content = @Content(schema = @Schema(implementation = UserDtos.PermissionsResponse.class))),
                        @ApiResponse(responseCode = "401", description = "Unauthorized - Authentication required", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
                        @ApiResponse(responseCode = "500", description = "Internal server error", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
        public ResponseEntity<UserDtos.PermissionsResponse> getPermissions(
                ...
                UserContext userContext = UserContext.fromJwt(jwt);
                UserPermissionsService.UserPermissions permissions = userPermissionsService
                ...
                UserDtos.PermissionsResponse response = UserDtos.PermissionsResponse.builder()
                                .allowedApiRoutes(permissions.getAccessibleApiRoutes() != null
                                                ? permissions.getAccessibleApiRoutes().stream().toList()
                                                : List.of())
                                .allowedUiRoutes(permissions.getAccessibleUiRoutes() != null
                                                ? permissions.getAccessibleUiRoutes().stream().toList()
                                                : List.of())
                                .roles(permissions.getRoles())
                                .teams(permissions.getTeamIds())
                                .features(Map.of()) // Placeholder for future features
                                .actions(permissions.getActions() != null ? permissions.getActions().entrySet().stream()
