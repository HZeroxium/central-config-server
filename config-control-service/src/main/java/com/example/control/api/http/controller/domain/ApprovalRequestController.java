...
import com.example.control.api.http.dto.domain.ApprovalRequestDtos;
import com.example.control.api.http.mapper.domain.ApprovalRequestApiMapper;
...
public class ApprovalRequestController {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "201", description = "Approval request created successfully", content = @Content(schema = @Schema(implementation = ApprovalRequestDtos.Response.class))),
                        ...
                        @ApiResponse(responseCode = "409", description = "Conflict - Request already exists", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
         ...
         */
        @GetMapping
        @Operation(summary = "List approval requests", description = """
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Approval requests retrieved successfully", content = @Content(schema = @Schema(implementation = ApprovalRequestDtos.ApprovalRequestPageResponse.class))),
        ...
        public ResponseEntity<ApprovalRequestDtos.ApprovalRequestPageResponse> findAll(
                ...
                ApprovalRequestDtos.ApprovalRequestPageResponse response = ApprovalRequestApiMapper
                                .toPageResponse(requests);
         ...
         */
        @GetMapping("/{id}")
        @Operation(summary = "Get approval request by ID", description = """
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Approval request found", content = @Content(schema = @Schema(implementation = ApprovalRequestDtos.Response.class))),
                        @ApiResponse(responseCode = "401", description = "Unauthorized - Authentication required", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
                        @ApiResponse(responseCode = "403", description = "Forbidden - Insufficient permissions", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
                        @ApiResponse(responseCode = "404", description = "Approval request not found", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
                        @ApiResponse(responseCode = "500", description = "Internal server error", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
        public ResponseEntity<ApprovalRequestDtos.Response> findById(
                        @Parameter(description = "Approval request ID", example = "request-12345") @PathVariable String id,
                        @AuthenticationPrincipal Jwt jwt) {
        ...
        @Operation(summary = "Submit approval decision", description = """
        ...
        public ResponseEntity<ApprovalRequestDtos.Response> submitDecision(
                        @Parameter(description = "Approval request ID", example = "request-12345") @PathVariable String id,
                        @Parameter(description = "Decision data (approve/reject with optional note)", schema = @Schema(implementation = ApprovalRequestDtos.DecisionRequest.class)) @Valid @RequestBody ApprovalRequestDtos.DecisionRequest request,
                        @AuthenticationPrincipal Jwt jwt) {
                log.info("Submitting decision for request: {} with decision: {}", id, request.decision());

                UserContext userContext = UserContext.fromJwt(jwt);

                // Auto-determine gate from user context
                String gate = determineGateForUser(userContext, id);
                if (gate == null) {
                        log.warn("User {} does not have permission to approve request {}", userContext.getUserId(), id);
        ...
        /**
         * Determine which gate the user can approve for based on their context and the
         * request.
         * 
         * @param userContext the user context
         * @param requestId   the approval request ID
         * @return gate name if user can approve, null otherwise
         */
        private String determineGateForUser(UserContext userContext, String requestId) {
                // Get the request to check gates
                Optional<ApprovalRequest> requestOpt = approvalService.findById(requestId, userContext);
                ...
                // Check SYS_ADMIN gate first (highest priority)
                if (userContext.hasRole("SYS_ADMIN")) {
                        boolean hasSysAdminGate = request.getRequired().stream()
                                        .anyMatch(gate -> "SYS_ADMIN".equals(gate.getGate()));
                        if (hasSysAdminGate && permissionEvaluator.canApprove(userContext, request, "SYS_ADMIN")) {
                                log.debug("User {} can approve for SYS_ADMIN gate", userContext.getUserId());
                                return "SYS_ADMIN";
                        }
                }

                // Check LINE_MANAGER gate
                boolean hasLineManagerGate = request.getRequired().stream()
                                .anyMatch(gate -> "LINE_MANAGER".equals(gate.getGate()));
                if (hasLineManagerGate && permissionEvaluator.canApprove(userContext, request, "LINE_MANAGER")) {
                        log.debug("User {} can approve for LINE_MANAGER gate", userContext.getUserId());
                        return "LINE_MANAGER";
                }
         ...
         */
        @DeleteMapping("/{id}")
        @Operation(summary = "Cancel approval request", description = """
        ...
        @ApiResponses(value = {
                        ...
                        @ApiResponse(responseCode = "401", description = "Unauthorized - Authentication required", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
                        @ApiResponse(responseCode = "403", description = "Forbidden - Not authorized to cancel this request", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
                        @ApiResponse(responseCode = "404", description = "Approval request not found", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
                        @ApiResponse(responseCode = "409", description = "Conflict - Request cannot be cancelled in current status", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
                        @ApiResponse(responseCode = "500", description = "Internal server error", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
