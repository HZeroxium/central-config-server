...
/**
 * Two-level {@link CacheManager} implementation that composes an L1 and an L2
 * cache manager.
 * <p>
 * <strong>Topology:</strong>
 * <ul>
 * <li><b>L1</b>: Local in-memory cache (e.g., Caffeine) for ultra-low latency
 * lookups.</li>
 * <li><b>L2</b>: Distributed cache (e.g., Redis) shared across application
 * instances.</li>
 * </ul>
 * <p>
 * <strong>Behavior:</strong>
 * <ul>
 * <li>{@link #getCache(String)} returns a composite {@link Cache} that first
 * attempts L1,
 * falls back to L2 on miss, and optionally promotes values from L2 to L1 for
 * faster subsequent access.</li>
 * <li>Writes go to L1 and—if configured via
 * {@link CacheProperties.TwoLevelConfig#isWriteThrough()}—also to L2.</li>
 * <li>When L2 is updated, publishes invalidation events via Redis pub/sub to
 * notify other instances.</li>
 * </ul>
 * <p>
 * This manager does <em>not</em> change semantics of Spring's caching
 * abstraction; it only orchestrates
 * the delegation to two underlying managers. For method-level semantics, see
 * Spring's {@link Cache}
...
public class TwoLevelCacheManager implements CacheManager {
    ...
    /**
     * L2 (distributed) cache manager, typically backed by Redis. May be
     * {@code null} when an L2
     ...
     * When present, publishes invalidation events to Redis when L2 is updated.
     */
     ...
     * <li>On L1 miss, attempt to read from L2 (if available); on L2 hit, promote
     * the value to L1.</li>
     * <li>On writes, always write to L1; optionally write to L2 when
     * {@code writeThrough} is enabled.</li>
     * </ol>
     * If the L1 cache cannot be resolved, the method logs a warning and returns the
     * L2 cache as the fallback.
     *
     * @param name the cache name
     * @return a composite two-level {@link Cache} when L1 is available; otherwise
     *         the L2 cache (may be {@code null})
     */
    @Override
     ...
     * <p>
     * <strong>Note:</strong> This implementation returns L1 names when present;
     * otherwise it returns L2 names
     * if available. It does <em>not</em> compute a strict union. In typical
     * deployments, both managers are
     * initialized with the same cache names. For a strict union, consider merging
     * the two name collections.
     *
    ...
    /**
     * Two-level {@link Cache} that orchestrates L1 (local) and L2 (distributed)
     * tiers.
     * <p>
     * <strong>Read path:</strong> try L1 → on miss try L2 → on L2 hit, promote to
     * L1. <br/>
     * <strong>Write path:</strong> write to L1; if {@code writeThrough} is enabled
     * and L2 is present, also write to L2.
     * <p>
     * Method semantics follow Spring's {@link Cache} contract, including:
     * <ul>
     * <li>{@link #get(Object)} and {@link #get(Object, Class)} read semantics and
     * type checks,</li>
     * <li>{@link #get(Object, Callable)} single-load semantics with error
     * propagation via {@code RuntimeException},</li>
     * <li>{@link #put(Object, Object)}, {@link #putIfAbsent(Object, Object)},</li>
     * <li>{@link #evict(Object)}, {@link #evictIfPresent(Object)},
     * {@link #clear()}, {@link #invalidate()}.</li>
     * </ul>
     * See the official Spring Javadoc for details of each method's contract (e.g.,
     * {@code evictIfPresent} vs {@code evict},
    ...
    public static class TwoLevelCache implements Cache {
        ...
        /**
         * Optional invalidation publisher for cross-instance invalidation.
         */
        private final CacheInvalidationPublisher invalidationPublisher;
         ...
         * the composite instance itself. Client code should not rely on
         * provider-specific
        ...
        /**
         * Retrieve a value for the given {@code key}, consulting L1 first and then L2
         * if necessary.
         * <p>
         * On an L2 hit, the value is promoted to L1 to accelerate subsequent reads.
         * Any exception from the underlying caches is caught and logged; the method
         * returns {@code null}
         * in such cases to preserve a non-failing read path.
         *
         * @param key the cache key
         * @return a {@link ValueWrapper} holding the value (which may itself be
         *         {@code null}),
         ...
         * <p>
         * Delegates to {@link #get(Object)} and performs a runtime type check in
         * accordance
         * with Spring's {@link Cache} contract; an {@link IllegalStateException} is
         * thrown when
         * a cached value is present but not assignable to {@code type}.
         *
         * @param key  the cache key
         * @param type the expected return type (may be {@code null} to bypass a type
         *             check)
         * @param <T>  inferred result type
         * @return the cached value cast to {@code T}, or {@code null} if not found
         * @throws IllegalStateException if a value is present but not of the required
         *                               type
         */
        @Override
        public <T> T get(Object key, Class<T> type) {
            ...
            if (type != null && value != null && !type.isInstance(value)) {
                throw new IllegalStateException(
                        "Cached value is not of required type [" + type.getName() + "]: " + value.getClass());
            }
         ...
         * <p>
         * Follows the standard "if cached, return; otherwise load, cache and return"
         * pattern.
         * Any exception thrown by the provided loader is wrapped in a
         * {@link RuntimeException} here.
         ...
         * <p>
         * Always writes to L1. If L2 is present and write-through is enabled via
         * configuration,
         * also writes to L2 to keep tiers aligned.
         *
         * @param key   the cache key
         * @param value the value to cache (may be {@code null} depending on cache
         *              configuration)
         */
        @Override
            ...
            try {
                ...
                if (l2Cache != null && config.isWriteThrough()) {
                    ...
                    if (invalidationPublisher != null && config.isInvalidateL1OnL2Update()) {
                        invalidationPublisher.invalidate(name, key.toString());
                    }
                }
            } catch (Exception e) {
         ...
         * <p>
         * Checks L1, then L2 (if available). If present in L2, promotes to L1.
         * Otherwise,
         * inserts a value (including into L2 via {@code putIfAbsent}) and finally
         * ensures L1
         * contains the value as a fast path. Errors are logged and {@code null} is
         * returned
         * according to the Spring {@link Cache} contract.
         *
         * @param key   the key
         * @param value the candidate value to associate
         * @return a {@link ValueWrapper} containing the existing value if one was
         ...
         * <p>
         * This method follows the best-effort semantics of Spring's
         * {@link Cache#evict(Object)}.
         *
         ...
         * <p>
         * For detailed semantics and immediate-visibility expectations, see
         * {@link Cache#evictIfPresent(Object)}.
         *
         * @param key the key to evict
         * @return {@code true} if an entry was known to be present before eviction;
         ...
         * <p>
         * Note that {@link #clear()} may be asynchronous or deferred depending on the
         * underlying provider;
        ...
        public void clear() {
            try {
                ...
                if (l2Cache != null) {
                    ...
                    // Publish invalidation event to notify other instances
                    if (invalidationPublisher != null) {
                        invalidationPublisher.clearAll(name);
                    }
                }
            } catch (Exception e) {
        ...
        /**
         * Invalidate the cache on both tiers with immediate post-conditions as defined
         * by Spring:
         * entries should be immediately invisible to subsequent lookups.
         * <p>
         * Returns {@code true} if both tiers report success (or the absent tier is
         * considered a no-op).
         * See {@link Cache#invalidate()} for details.
         *
         * @return {@code true} if the cache was invalidated successfully; {@code false}
         *         otherwise
         */
        @Override
