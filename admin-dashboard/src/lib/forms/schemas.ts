...
export const applicationServiceCreateSchema = z.object({
  ...
  ownerTeamId: z.string().min(1, 'Owner team is required').max(100, 'Owner team ID must not exceed 100 characters'),
  environments: z.array(z.string()).min(1, 'At least one environment must be specified'),
  tags: z.array(z.string()).optional(),
  repoUrl: z.string().max(500, 'Repository URL must not exceed 500 characters').url('Must be a valid URL').optional().or(z.literal('')),
...
export const approvalRequestCreateSchema = z.object({
  serviceId: z.string().min(1, 'Service ID is required').max(100, 'Service ID must not exceed 100 characters'),
  targetTeamId: z.string().min(1, 'Target team is required').max(100, 'Target team ID must not exceed 100 characters'),
...
export const serviceShareCreateSchema = z.object({
  ...
  grantToId: z.string().min(1, 'Grantee ID is required').max(100, 'Grantee ID must not exceed 100 characters'),
 ...
 */
export type ApplicationServiceCreateInput = z.infer<typeof applicationServiceCreateSchema>;
export type ApplicationServiceUpdateInput = z.infer<typeof applicationServiceUpdateSchema>;
...
export type ServiceInstanceUpdateInput = z.infer<typeof serviceInstanceUpdateSchema>;
