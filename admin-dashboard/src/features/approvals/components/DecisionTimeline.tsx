...
export const DecisionTimeline: React.FC<DecisionTimelineProps> = ({ request }) => {
  ...
  const decisions = ((request as any).decisions || []) as Array<{
    decidedAt?: string;
    decision: string;
    decidedBy?: string;
    note?: string;
              ...
              <Box sx={{ flex: 1 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    {event.timestamp ? format(new Date(event.timestamp), 'MMM dd, yyyy HH:mm') : 'Unknown'}
                  </Typography>
                </Box>
