...
export default function ConfigMergeInfo({
  env,
}: Readonly<ConfigMergeInfoProps>) {
  ...
  const sources = env.propertySources.map((ps, index) => {
    ...
    return {
      ...
      isGlobal:
        sourceName.includes("/application.yml") &&
    ...
    <Card sx={{ border: 1, borderColor: "divider", boxShadow: 1 }}>
      <CardContent sx={{ p: 3 }}>
        ...
        <Box sx={{ mt: 3, p: 2, bgcolor: "grey.50", borderRadius: 2 }}>
          ...
          >
            • Properties from higher priority sources override lower priority
            ones
            <br />
            • Missing properties are inherited from lower priority sources
            <br />• Final configuration combines all non-overridden properties
          </Typography>
        </Box>
      </CardContent>
    </Card>
