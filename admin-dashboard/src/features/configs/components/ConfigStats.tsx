...
interface ConfigStatsProps {
  readonly env: ConfigEnvironmentResponse;
}
...
export default function ConfigStats({ env }: ConfigStatsProps) {
  const stats = useMemo(() => {
    ...
    const totalProperties = sources.reduce(
      (acc, source) => acc + Object.keys(source.source || {}).length,
      0
    );

    const globalSources = sources.filter(
      (s) =>
        s.name?.includes("/application.yml") && !s.name?.includes("/application-")
    ).length;
    const profileSources = sources.filter(
      (s) =>
        s.name?.includes("/application-") && !s.name?.includes("/application.yml")
    ).length;
    const serviceSources = sources.filter(
      (s) =>
        !s.name?.includes("/application.yml") &&
        !s.name?.includes("/application-")
    ).length;
