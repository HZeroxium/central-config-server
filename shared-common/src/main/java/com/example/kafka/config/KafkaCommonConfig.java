...
public class KafkaCommonConfig {
    ...
    public ProducerFactory<String, String> producerFactory(KafkaProperties props) {
        ...
        String txPrefix = props.getProducer().getTransactionIdPrefix();
        if (txPrefix == null || txPrefix.isBlank()) {
            // Provide a sensible default so KafkaTemplate can participate in container transactions
            txPrefix = "tx-";
        }
    ...
    public ConcurrentKafkaListenerContainerFactory<String, String> txFactory(
        ...
        // Producer transactions are explicitly started via KafkaTemplate.executeInTransaction in handlers
