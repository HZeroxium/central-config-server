...
# Load minimal variables to compute port mappings without exporting to current shell
...
docker run -d \
  ...
  hzeroxium/user-rest-spring-service:latest

echo "Started $SERVICE_NAME on :$HOST_HTTP_PORT"
