...
/**
 * REST facade acting as a Thrift client. Accepts
 * {@link com.example.rest.user.dto.UserRequest}
 * and returns {@link com.example.rest.user.dto.UserResponse}, while delegating
 * to the underlying
...
public class UserController {
  ...
  @ApiResponse(responseCode = "200", description = "Created", content = @Content(schema = @Schema(implementation = CreateUserResponse.class)))
  ...
  @ApiResponse(responseCode = "200", description = "Found", content = @Content(schema = @Schema(implementation = GetUserResponse.class)))
  ...
  @ApiResponse(responseCode = "200", description = "Updated", content = @Content(schema = @Schema(implementation = UpdateUserResponse.class)))
  @ApiResponse(responseCode = "404", description = "Not Found")
  @Timed(value = MetricsConstants.REST_API_UPDATE_USER, description = MetricsConstants.UPDATE_DESCRIPTION)
  ...
  @ApiResponse(responseCode = "200", description = "Deleted", content = @Content(schema = @Schema(implementation = DeleteUserResponse.class)))
  @ApiResponse(responseCode = "404", description = "Not Found")
  @Timed(value = MetricsConstants.REST_API_DELETE_USER, description = MetricsConstants.DELETE_DESCRIPTION)
  public ResponseEntity<DeleteUserResponse> delete(@PathVariable @Parameter(description = "User id") String id) {
    ...
    if (existingUser.isEmpty()) {
      ...
      var response = ResponseBuilder.buildUserNotFoundForDeleteResponse();
      return ResponseEntity.status(StatusCode.getHttpStatus(StatusCode.USER_NOT_FOUND)).body(response);
    }
    userService.delete(id);
    log.info("User deleted successfully with ID: {}", id);
    var response = ResponseBuilder.buildDeleteUserResponse();
    return ResponseEntity.ok(response);
  }

  @GetMapping
  @Operation(summary = "List users with advanced query support")
  @ApiResponse(responseCode = "200", description = "OK", content = @Content(schema = @Schema(implementation = ListUsersResponse.class)))
  @Timed(value = MetricsConstants.REST_API_LIST_USERS, description = MetricsConstants.LIST_BY_CRITERIA_DESCRIPTION)
  ...
  public ResponseEntity<ListUsersResponse> list(@Validated ListUsersRequest request) {
    ...
    var response = ResponseBuilder.buildListUsersResponse(items, request.getPage(), request.getSize(), total,
        totalPages);
