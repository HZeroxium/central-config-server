...
class UserServiceIntegrationTest {
    ...
    class ListUsersIntegrationTests {
        ...
        void shouldListUsersWithPaginationSuccessfullyWithRealSpringContext() {
            ...
            when(userRepository.findPage(page, size)).thenReturn(expectedUsers);
        ...
        @Test
        @DisplayName("Should handle list users database failure with real Spring context")
        void shouldHandleListUsersDatabaseFailureWithRealSpringContext() {
            // Given
            int page = 0;
            int size = 10;
            DatabaseException cause = new DatabaseException("Database query failed", "listPaged", new RuntimeException("Query failed"));
            when(userRepository.findPage(page, size)).thenThrow(cause);
