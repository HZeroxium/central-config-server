...
class UserServiceHandlerTest {
    ...
    class ListUsersTests {
        ...
        void shouldListUsersSuccessfully() throws TException {
            // Given
            ...
            when(userService.list(0, 10)).thenReturn(users);
            when(userService.count()).thenReturn(2L);
            

            // When
            TListUsersResponse response = handler.listUsers(request);
        ...
        void shouldHandleEmptyUserList() throws TException {
            // Given
            TListUsersRequest request = new TListUsersRequest()
                    .setPage(0)
                    .setSize(10);

            when(userService.list(0, 10)).thenReturn(List.of());
        ...
        @Test
        @DisplayName("Should handle database error during list users")
        void shouldHandleDatabaseErrorDuringListUsers() throws TException {
            // Given
            TListUsersRequest request = new TListUsersRequest()
                    .setPage(0)
                    .setSize(10);
            ...
            DatabaseException exception = new DatabaseException("Database error");
            when(userService.list(0, 10)).thenThrow(exception);
            ...
            // Then
            assertThat(response.getStatus()).isEqualTo(2);
            assertThat(response.getMessage()).isEqualTo("Database error during user listing: Database error");
            assertThat(response.getItems()).isEmpty();
            assertThat(response.getPage()).isEqualTo(0);
            assertThat(response.getSize()).isEqualTo(10);
            assertThat(response.getTotal()).isEqualTo(0L);
        ...
        void shouldCalculateTotalPagesCorrectly() throws TException {
            // Given
            TListUsersRequest request = new TListUsersRequest()
                    .setPage(0)
                    .setSize(3);
            ...
            when(userService.list(0, 3)).thenReturn(users);
            when(userService.count()).thenReturn(10L); // 10 total users, 3 per page = 4 pages
    ...
    class LegacyApiTests {
        ...
        @Test
        @DisplayName("Should handle legacy ping successfully")
        void shouldHandleLegacyPingSuccessfully() throws TException {
            // Given
            String expectedResponse = "pong";
            when(userService.ping()).thenReturn(expectedResponse);
            ...
            // When
            String response = handler.pingLegacy();
        ...
        @Test
        @DisplayName("Should handle legacy ping with exception")
        void shouldHandleLegacyPingWithException() {
            // Given
            UserServiceException exception = new UserServiceException("SERVICE_ERROR", "Service unavailable");
            when(userService.ping()).thenThrow(exception);
            ...
            // When & Then
            assertThatThrownBy(() -> handler.pingLegacy())
                    .isInstanceOf(TException.class)
                    .hasMessage("Service ping failed: Service unavailable");
            
            
        }

        @Test
        @DisplayName("Should handle legacy create user successfully")
        void shouldHandleLegacyCreateUserSuccessfully() throws TException {
            // Given
            TUser thriftUser = new TUser()
                    .setName("John Doe")
                    .setPhone("+1-555-123-4567")
                    .setAddress("123 Main St");
            ...
            // When
            TUser response = handler.createUserLegacy(thriftUser);
        ...
        @Test
        @DisplayName("Should handle legacy get user successfully")
        void shouldHandleLegacyGetUserSuccessfully() throws TException {
            // Given
            ...
            when(userService.getById(userId)).thenReturn(Optional.of(foundUser));
            

            // When
            TUser response = handler.getUserLegacy(userId);
        ...
        @Test
        @DisplayName("Should handle legacy get user not found")
        void shouldHandleLegacyGetUserNotFound() throws TException {
            // Given
        ...
        @Test
        @DisplayName("Should handle legacy delete user successfully")
        void shouldHandleLegacyDeleteUserSuccessfully() throws TException {
            // Given
            String userId = "user-123";
            

            // When
            handler.deleteUserLegacy(userId);
        ...
        @Test
        @DisplayName("Should handle legacy list users successfully")
        void shouldHandleLegacyListUsersSuccessfully() throws TException {
            // Given
            int page = 0;
            int size = 10;
            List<User> users = List.of(
            ...
            when(userService.list(page, size)).thenReturn(users);
            when(userService.count()).thenReturn(2L);
            

            // When
            TPagedUsers response = handler.listUsersLegacy(page, size);
            ...
            verify(userService).list(page, size);
            verify(userService).count();
        }
    }
    ...
    class DataMappingTests {
        ...
        @Test
        @DisplayName("Should map thrift user to domain user correctly")
        void shouldMapThriftUserToDomainUserCorrectly() throws TException {
            // Given
            TUser thriftUser = new TUser()
                    .setId("user-123")
                    .setName("John Doe")
            ...
            // Then
            assertThat(response.getId()).isEqualTo(thriftUser.getId());
            assertThat(response.getName()).isEqualTo(thriftUser.getName());
            assertThat(response.getPhone()).isEqualTo(thriftUser.getPhone());
            assertThat(response.getAddress()).isEqualTo(thriftUser.getAddress());
        }
